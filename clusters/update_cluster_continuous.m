...
function [membership,betaY,sigma2Y,betaT,x_pis,x_means,...
    x_vars] = update_cluster_continuous(T,X,Y,p1,p2,z,beta_coeff,sigma2_coeff,gamma_coeff,x_pi_param,x_mean_param,...
...
sigma2_aux_table = 1.0./gamrnd(sigma2_shape,1/sigma2_scale,m * n,1);
beta_table_idx = 1;
...
for ii = randperm(n)
    ...
    loglikeregy_olds = lognormpdf(Y(ii),sum(TX_matrix(ii,:) .* beta_coeff,2),sqrt(sigma2_coeff));
    ...
    for jj = 1:k
        ...
        %t_clst_idx = sort(uniqueZyt(uniqueZyt(:,1) == tmp_y_idx,2));
        kj = kjs(jj);
        ...
        %         loglikeregy = -normlike([TX_matrix(ii,:) * beta_coeff(jj,:)',sqrt(sigma2_coeff(jj,:))],Y(ii));
...
%     sigma2_coeff_aux = 1.0./gamrnd(sigma2_shape,1/sigma2_scale,m,1);
%     
...
%            fprintf('beta fantasy');
      ...
      sigma2_coeff_aux = sigma2_aux_table(beta_table_idx:beta_table_idx+m-1,:);
...
%             fprintf('xvars fantasy \n');
    ...
    loglikeregy_new = lognormpdf(Y(ii),sum(TX_matrix(ii,:) .* beta_coeff_aux,2),sqrt(sigma2_coeff_aux));
    ...
    for jj = 1:m
        ...
        %         loglikeregy = -normlike([TX_matrix(ii,:) * beta_coeff_aux(jj,:)',sqrt(sigma2_coeff_aux(jj,:))],Y(ii));
    ...
    if newCluster < newXidx
        ...
        if  newCluster >=  newYidx
            ...
            sigma2_coeff = [sigma2_coeff; sigma2_coeff_aux(tmpidx_aux,:)];
            ...
            elseif newCluster >= newXidx
                ...
                for jj = 1:k
                    tmp_y_idx = jj;
                    % get count of number of T clusters within jth Y cluster
                    kj = length(uniqueZyt(uniqueZyt(:,1) == tmp_y_idx,2));
                    yidxs(jj) =  kj*m;
                end
                yidxs = cumsum(yidxs);
