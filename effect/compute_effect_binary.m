function [new_psi_rr,new_psi_rd] = compute_effect_binary(X,n,p1,p2,z,k,kjs,kljs,njs,nljs,nhljs,beta_coeff,gamma_coeff,x_pi_param,x_mean_param,x_var_param,...
...
%         [z_mc,X_mc,k_mc,Njs_mc,Nljs_mc,Nhljs_mc,beta_coeff_mc,gamma_coeff_mat,x_pi_param_mat,x_mean_param_mat,x_var_param_mat] ...
%     = samplePosterior(X,n,p1,p2,k,kjs,kljs,njs,nljs,nhljs,beta_coeff,gamma_coeff_mat,x_pi_param_mat,x_mean_param_mat,x_var_param_mat,...
%     alpha_theta,alpha_psi,alpha_omega,M,beta_coeff0,diagbetacov0,gamma_coeff0,diaggammacov0,c0, mu0, nu0, tau0, a0, b0);
%
...
e0yt1_mc = mean(expit(TXt1_matrix_mc * beta_coeff_prior_mc'),2);
e0yt0_mc = mean(expit(TXt0_matrix_mc * beta_coeff_prior_mc'),2);
...
%         tmpht0x = binopdf(zeros(size(X_mc,1),1),1,prob_t);
    ...
    partt1_mc = partt1_mc + wt1_mc(:,jj) .* expit(TXt1_matrix_mc * beta_coeff_mc(jj,:)');
    partt0_mc = partt0_mc + wt0_mc(:,jj) .* expit(TXt0_matrix_mc * beta_coeff_mc(jj,:)');
end % for jj
...
new_psi_rr = mean(numt1_mc./denomt1_mc) / mean(numt0_mc./denomt0_mc);
