function results = scen3_ContinuousOutcome(seed,n,run_ID,isdisplay)
...
% The difference with binary outcome
%  F(y|t,x,\theta_{j})
%  E_{0}[y|t,x]
%  E[y|t,x,\theta_{j}^{*}]
...
inputfile = ['datasets/scen3/scen3_n'  num2str(n)   '_run_ID'  num2str(run_ID) '.txt'];
...
%% Hyperparameters-----------------------------------------------------------------
% for modeling continouse outcomes
sigma2_shape = 1; % shape
sigma2_scale = 1; % scale (inverse scale for gamma distribution)
...
resultsfile = ['results/scen3/cedp_scen3_n'  num2str(n)   '_run_ID'  num2str(run_ID) '.mat'];
workplacefile = ['results/scen3_workplace/cedp_scen3_n'  num2str(n)   '_run_ID'  num2str(run_ID) '.mat'];

%% Update values of parameters-----------------------------------------------------
% % For outcome regression coefficients parameters in outcome regression.
[beta_coeff0,diagbetacov0,precbetacov0, sigma2_coeff,beta_coeff] = init_beta_params_norm(Y,TX,TX_matrix,z,k,sigma2_shape,sigma2_scale);
...
for gibbsreps = 1:GIBBS_TOTAL
    ...
    [z,beta_coeff,sigma2_coeff,gamma_coeff,x_pi_param,x_mean_param,...
        x_var_param] = update_cluster_continuous(T,X,Y,p1,p2,z,beta_coeff,sigma2_coeff,gamma_coeff,x_pi_param,x_mean_param,...
        x_var_param,alpha_theta,alpha_psi,alpha_omega,NUM_AUX_PARAM,beta_coeff0, diagbetacov0,sigma2_shape,sigma2_scale,...
 ...
 %  Calculate number of clusters.
    ...
    %% Update parameters
    % Update beta and sigma2 parameters
    ...
    yclusNum(gibbsreps) = k;
      loglikeregData(gibbsreps) = computeloglike_continous(T,X,Y,TX_matrix,X_matrix,p1,p2,...
        z,k,kjs,kljs,beta_coeff,sigma2_coeff,gamma_coeff,x_pi_param,x_mean_param,x_var_param);
 ...
 %% Update concentration parameters--------------------------------------------
    ...
    % Start----------------------------------------------------------------------
     if gibbsreps >= BURN_IN && mod(gibbsreps,100) == 0
         ...
         psi_rd(gibbsreps,1) = compute_effect_continuous(X,n,p1,p2,z,k,kjs,kljs,njs,nljs,nhljs,...
...
% psi_rd_true = 10.78; % for linear
psi_rd_true = 1.503;

[median_psi_rd,sd_psi_rd,cov_psi_rd,width_psi_rd] = compute_psi_rd_stats(psi_rd_true,psi_rd);

% Save results to a vector
results = [median_alpha_theta,median_alpha_psi,median_alpha_omega,median_psi_rd,sd_psi_rd,cov_psi_rd,width_psi_rd];
