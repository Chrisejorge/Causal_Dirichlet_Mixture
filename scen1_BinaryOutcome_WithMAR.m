function results = scen1_BinaryOutcome_WithMAR(seed,n,run_ID,isdisplay)
...
% Indicators for missing data, 1 is missing
X_mar = zeros(n,p1+p2);

X_mar(:,1) = binornd(1, expit(-2 + X(:, 2) + Y),n,1);
X_mar(:,2) = binornd(1, expit(-2 + X(:, 3) + T),n,1);
X_mar(:,3) = binornd(1, expit(-1.5 - T + Y),n,1);
X_mar(:,4) = binornd(1, expit(-0.9 - X(:, 1) - X(:, 2)),n,1);
...
resultsfile = ['results/scen1_mar/cedp_scen1_mar_n'  num2str(n)   '_run_ID'  num2str(run_ID) '.mat'];
workplacefile = ['results/scen1_mar_workplace/cedp_scen1_mar_n'  num2str(n)   '_run_ID'  num2str(run_ID) '.mat'];
...
for gibbsreps = 1:GIBBS_TOTAL
   ...
   if mod(gibbsreps,1000) == 0
    ...
    [X,TX,X_matrix,TX_matrix] = update_missing(T,X,Y,TX_matrix,pt,p1,p2,X_mar,...
        z,k,kjs,kljs,beta_coeff,x_pi_param,x_mean_param,x_var_param);    
